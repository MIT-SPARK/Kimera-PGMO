...
#include "kimera_pgmo_ros/VoxbloxMsgInterface.h"
...
MeshFrontend::MeshFrontend(const Config& config, const ros::NodeHandle& nh)
  ...
  // (voxblox) mesh input
  sub_ = nh_.subscribe("mesh_in", config.queue_size, &MeshFrontend::handleMesh, this);
  ROS_INFO("Initialized MeshFrontend.");
}

void MeshFrontend::handleMesh(const voxblox_msgs::Mesh& msg) {
  const VoxbloxMsgInterface mesh(&msg);
  update(mesh, msg.header.stamp.toSec());
