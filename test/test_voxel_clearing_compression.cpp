...
struct CompressionInputs {
  CompressionInputs()
      : vertices(new Cloud()), triangles(new std::vector<pcl::Vertices>()),
...
bool checkTriangles(const std::vector<std::vector<uint32_t>> &expected,
  ...
  for (size_t i = 0; i < expected.size(); ++i) {
    ...
    std::set<uint32_t> result_set(result_filtered[i].begin(),
                                  result_filtered[i].end());
...
TEST(test_voxel_clearing_compresssion, clearingCorrectBasic) {
  ...
  { // limit temporary scopes
    ...
    compression.compressAndIntegrate(mesh, input.vertices, input.triangles,
                                     input.indices, input.remappings, 100.0);
...
TEST(test_voxel_clearing_compresssion, pruningCorrectBasic) {
  VoxelClearingCompression compression(compression_factor);
  ...
  { // limit temporary scopes
    ...
    compression.compressAndIntegrate(mesh, input.vertices, input.triangles,
                                     input.indices, input.remappings, 105.0);
...
TEST(test_voxel_clearing_compresssion, clearingCorrectMultiblock) {
  VoxelClearingCompression compression(compression_factor);
  ...
  { // limit temporary scopes
    ...
    EXPECT_TRUE(
        checkTriangles({{6, 7, 8}, {3, 4, 5}, {0, 1, 2}}, *output.triangles));
  }
  ...
  { // limit temporary scopes
    ...
    EXPECT_TRUE(
        checkTriangles({{6, 7, 8}, {3, 4, 5}, {9, 10, 11}}, *output.triangles));
  }
...
TEST(test_voxel_clearing_compresssion, pruningCorrectMultiblock) {
  VoxelClearingCompression compression(compression_factor);
  ...
  { // limit temporary scopes
    ...
    // re-integrating an empty block should not add anything
    compression.compressAndIntegrate(mesh, input.vertices, input.triangles,
                                     input.indices, input.remappings, 102.0);
  ...
  { // limit temporary scopes
    ...
    EXPECT_TRUE(checkTriangles(
        {{0, 1, 2}, {3, 4, 5}, {9, 10, 11}, {12, 13, 14}}, *output.triangles));
  }
}
