...
def read_node_line(line_str):
    ...
    return int(entries[1]), [float(entries[2]), float(entries[3]), float(entries[4])]


def read_edge_line(line_str):
    entries = line_str.split()
    return [int(entries[1]), int(entries[2])]
...
def main():
    parser = argparse.ArgumentParser(description="utiltiy to plot a deformation graph.")
    parser.add_argument("dgrf", type=str, help="input .dgrf file.")
    args = parser.parse_args()

    nds, btns, tbtns, degs, tdegs = read_dgrf_file(args.dgrf)

    all_pos = np.array(list(nds.values()))
